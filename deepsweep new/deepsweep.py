...
"""
    ...
    def check_ssrf(self):
        ...
        try:
            test_urls = [f"{self.target_url}?url=http://testphp.vulnweb.com", f"{self.target_url}?fetch=http://testphp.vulnweb.com"]
            ...
            for url in test_urls:
                for payload in ssrf_payloads:
                    try:
                        response = self.session.get(url.replace('http://testphp.vulnweb.com', quote(payload)), timeout=self.timeout, verify=False)
